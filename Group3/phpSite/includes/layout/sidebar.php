...
if (isset($_POST['insertSubscriber'])) {
  ...
  if (empty($_POST['name'])) {
    $invalidName = "درج نام الزامیست";
  }
  if (empty($_POST['email'])) {
    $invalidEmail = "درج ایمیل الزامیست";
  }
  ...
  if (!empty($_POST['name']) && !empty($_POST['email'])) {
    ...
    $successfullSubscriberInsert = "ثبت نام با موفقیت انجام شد";
  }
}
...
<div class="col-lg-4">
  ...
  <div class="card mt-4">
    ...
    <ul class="list-group list-group-flush p-0">
      <?php foreach ($categories as $categotry): ?>
        <li class="list-group-item">
          <a class="link-body-emphasis text-decoration-none" href="index.php?id=<?= $categotry['id'] ?>">
            <?= $categotry['title'] ?>
          </a>
        </li>
      <?php endforeach ?>
    ...
    <div class="card-body">
      <form method="post">
        ...
        <div class="d-grid gap-2">
          ...
          <p class="text-success text-center"><?= $successfullSubscriberInsert ?></p>
        </div>
      </form>
    </div>
  </div>
